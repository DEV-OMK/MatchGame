...
class MatchGame extends Component {
  ...
  toggleMatchImage = id => {
    const {matchImage, score, gameOver} = this.state

    if (matchImage.id === id) {
      let index = Math.floor(Math.random() * imagesList.length)

      while (
        imagesList[index].category === matchImage.category ||
        imagesList[index].id === matchImage.id
      ) {
        index = Math.floor(Math.random() * imagesList.length)
      }

      this.setState({
        matchImage: imagesList[index],
        score: score + 1,
      })
    } else {
      clearInterval(this.intervalId)
  ...
  render() {
    const {matchImage, activeTabId, timeRemaining, score, gameOver} = this.state
    ...
    return (
      <>
        <Navbar timer={timeRemaining} score={score} />
        <div className="bg-container">
              ...
              <ul className="thumbnail-list-container">
                {filteredImagesList.map(eachImage => (
                  <ThumbnailItem
                    ...
                    toggleMatchImage={this.toggleMatchImage}
                  />
                ))}
              </ul>
            </div>
          ...
          {gameOver && (
            <div className="responsive-container">
              <GameOverItem score={score} resetGame={this.resetGame} />
            </div>
          )}
        </div>
      </>
